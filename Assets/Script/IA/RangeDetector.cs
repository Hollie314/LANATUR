...
public class RangeDetector : MonoBehaviour
{
    ...
    public void ChangePlayerDetected(GameObject newObj, bool add, bool crouchTrigger, bool charCrouched)
    {
        if (add)
        {
            if (GameObjectsDetected.Contains(newObj)) // si le joueur est d�j� d�tect�, sortir
            {
                ////Debug.Log("Joueur d�j� d�tect� - return");
                return;
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        ////Debug.Log("Joueur dans crouch trigger - ajout�");
                        GameObjectsDetected.Add(newObj);
                        return;
                    }
        ...
        {
            if (!GameObjectsDetected.Contains(newObj)) // Si le joueur n'�tait d�j� pas pr�sent, sortir
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        ////Debug.Log("Joueur sorti de base trigger - retir�");
                        GameObjectsDetected.Remove(newObj);
                        return;
                    }
                    else { return ; } // Sinon sortir
