...
namespace RTGStandard
{
    ...
    public class RTGizmos : MonoSingleton<RTGizmos>
    {
        ...
        int             mFrameCount             = 0;                        // Current frame count
        #endregion
        ...
        public void Internal_Update()
        {
            ...
            // Consume pick button up if button was released over a hovered gizmo
        ...
        public void Internal_Render(Camera camera, RasterGraphContext rasterCtx)
        {
            ...
            if (mFrameCount != Time.frameCount)
            {
        ...
        void OnGUI()
        {
            ...
            //-----------------------------------------------------------------------------
            // Clear the list of render cameras after the GUI has been drawn for this frame.
            //
            // In older Unity versions, camera rendering and OnGUI execution were often
            // interleaved (e.g. CameraA render -> OnGUI, CameraB render -> OnGUI), which
            // implicitly tied each OnGUI call to the last rendered camera.
            //
            // In newer Unity versions, camera rendering and OnGUI execution are decoupled:
            // all cameras may render first, and OnGUI is then invoked globally (once per
            // event type). Because OnGUI no longer has a reliable per-camera context, we
            // collect the cameras that rendered gizmos during the frame and draw all
            // gizmo GUI in the first Repaint pass. The list is then cleared so that
            // subsequent OnGUI invocations in the same frame (e.g. input events) do not
            // redraw or reprocess GUI state.
            //-----------------------------------------------------------------------------
