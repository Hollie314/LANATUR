...
float _BaseTexturesTriplanar;
float _BaseTexturesTriplanarSharpness;
...
void UnpackTrackSplatColor(out float4 trackSplatsColor[_LAYER_COUNT])
{
    ...
    #if defined(_NORMALMAPS) && !defined(_TERRAIN_NORMAL_IN_MASK) 
        #define SampleNormals(i, uv) (UnpackNormalWithScale(UNITY_SAMPLE_TEX2D_SAMPLER(_Normal##i, _Splat0, uv), _NormalScale##i).xyz)
    ...
    float3 RainRipples(float2 uv, float scale, float rotation)
    {
        ...
        #ifdef DIFFUSE           
            #define Mask(i, uv) UNITY_SAMPLE_TEX2D_SAMPLER(_Splat##i, _Splat0, uv)
        #else
            #ifdef TERRAIN_MASK
                #define Mask(i, uv) UNITY_SAMPLE_TEX2D_SAMPLER(_Mask##i, _Splat0, uv)
            #else
        ...
        #endif
    #else
        #define Mask(i, uv) float4(_Metallic##i, 1.0f, 0.5f, 0.0f);
        #define RemapMask(i, mask) mask;
    #endif
    ...
    void SampleDistantMask(half4 mask[_LAYER_COUNT], out half4 distMask[_LAYER_COUNT], float2 uvR[_LAYER_COUNT], float2 uvG[_LAYER_COUNT], float2 uv[_LAYER_COUNT], half4 blendMask[2], int mode[_LAYER_COUNT], inout float2 stochasticMask[_LAYER_COUNT], float weight)
    {
        ...
        UNITY_BRANCH if (blendMask * weight > 0.0f && mode[i] != 3)     \
        {                                                               \
            distMask[i] = sMask.r * RemapMask(i, Mask(i, uvR[i]));      \
            UNITY_BRANCH if (mode[i] > 0)                               \
            {                                                           \
                distMask[i] +=  sMask.g * RemapMask(i, Mask(i, uvG[i]));\
            }                                                       \
        }                                                           \
        else                                                        \
        {                                                           \
    ...
    #undef SampleDistMask
    }

    #ifndef DIFFUSE
        #define SampleSplt(i, uv) UNITY_SAMPLE_TEX2D_SAMPLER(_Splat##i, _Splat0, uv)
    #else
        #define SampleSplt(i, uv) mask[i]
    #endif   
    ...
    void SampleDistantSplat(float2 uvR[_LAYER_COUNT], float2 uvG[_LAYER_COUNT], float2 uv[_LAYER_COUNT], half4 blendMask[2], float2 stochasticMask[_LAYER_COUNT], float weight, inout half4 mask[_LAYER_COUNT], int mode[_LAYER_COUNT], out float4 mixAlbedo, out float3 mixNormal, in out half coverMask)
    { 
        float3 normal[_LAYER_COUNT]; 
        ...
        {                                                               \
            albedoR = SampleSplt(i, uvR[i]);                            \
            albedoR.rgb *= DiffuseRemap(i).xyz;                         \
            albedoR.a = Smoothness(i, albedoR).x;                       \
            albedoR *= sMask.r;                                         \
            albedo[i] = albedoR;                                        \
            normal[i] = SampleNormals(i, uvR[i]).xyz * sMask.r;         \
            UNITY_BRANCH if (mode[i] > 0)                               \
            {                                                           \
                albedoG = SampleSplt(i, uvG[i]);                        \
                albedoG.rgb *=  DiffuseRemap(i).xyz;                    \
                albedoG.a = Smoothness(i, albedoG).x;                   \
                albedo[i] = albedoR + (albedoG * sMask.g);              \
                normal[i] += SampleNormals(i, uvG[i]) * sMask.g;        \
            }                                                           \
            coverMask += blendMask;                                     \
            mixAlbedo += albedo[i].xyzw * blendMask;                    \
            mixNormal += normal[i].xyz * blendMask;                     \
    ...
    void CoverMaskAndMode(half4 blendMask[2], out float coverMask, out int mode[_LAYER_COUNT])
    {
        coverMask = 0;

        #define cover(i, blendMask)                         \
        ...
        cover(0, blendMask[0].r);
        #ifndef _LAYERS_ONE
            cover(1, blendMask[0].g);
            #ifndef _LAYERS_TWO
            if (_TwoLayersOnly < 1)
            {
                cover(2, blendMask[0].b);
                cover(3, blendMask[0].a);
                #ifdef _LAYERS_EIGHT
                    cover(4, blendMask[1].r);
                    cover(5, blendMask[1].g);
                    cover(6, blendMask[1].b);
                    cover(7, blendMask[1].a);
                #endif
            }
            #endif
        #endif
    ...
    {   
        ...
        void SampleDistantSplatTOL(in out half4 mixedAlbedo, in out half3 mixedNormal, float2 uvR[_LAYER_COUNT], float2 uvG[_LAYER_COUNT], half4 blendMask[2], float2 stochasticMask[_LAYER_COUNT], float weight, half4 mask[_LAYER_COUNT], int mode[_LAYER_COUNT], in out half coverMask)
        {
            float4 albedo[1];
            float3 normal[1];
             ...
             blendMask[0].r *= weight;
                ...
                {
                    albedoR = SampleSplt(0, uvR[0]);
                    albedoR.rgb *= DiffuseRemap(0).xyz;
                    albedoR.a = Smoothness(0, albedoR);
                    albedoR *= sMask.r;
                    albedo[0] = albedoR;
                    ...
                    if (mode[0] > 0)
                    {
                        albedoG = SampleSplt(0, uvG[0]);
                        albedoG.rgb *= DiffuseRemap(0).xyz;
                        albedoG.a = Smoothness(0, albedoG);
                        albedoG *= sMask.g;
