...
void SplatmapVert(inout appdata_full v, out Input data)
{
    ...
    float3 wDir = WorldSpaceViewDir(v.vertex);
        ...
        float heightOffset = data.worldPos.y - _TerrainPosition.y + (height * -_TerrainHeightmapScale.y);
            ...
            {
                v.normal = UnityObjectToWorldNormal(terrainNormal);
            }
            if (_CheckHeight)
            {
                heightOffset = _TerrainPosition.y - (height * -_TerrainHeightmapScale.y);
                float3 worldVertPos = float3(data.worldPos.x, heightOffset, data.worldPos.z);
                v.vertex.y = mul(unity_WorldToObject, worldVertPos).y; 
            }
        #endif
...
#endif
{
    ...
    //================================================================
        ...
        float3  tWeights = abs(wNormal);
        tWeights = pow(tWeights, _TriplanarSharpness);
        tWeights = tWeights / (tWeights.x + tWeights.y + tWeights.z);

        #if defined(INTERRA_OBJECT)
            float3  baseTriplanarWeights = pow(tWeights, _BaseTexturesTriplanarSharpness);
        ...
        #else
            TriplanarOneToAllSteep(blendMask, (1 - tWeights.y), weight);
        #endif   
    #endif
    ...
    half4 origBlendMask[2] = blendMask;
        ...
        float distBlend = smoothstep(_HT_distance.x, _HT_distance.y, dist);
        ...
        float sampleDistMaskFullCover = (dist > _HT_distance.y) && coverMask > 0.99f && _HT_cover > 0.99f ? 1 : 0; 
        #ifdef INTERRA_OBJECT
        ...
        {
            mainUV = TRANSFORM_TEX(IN.worldPos.xz, _MainTex);
        }
        else
        {
            mainUV = TRANSFORM_TEX(IN.mainTC_tWeightY_hOffset.xy, _MainTex);
        }
        ...
        float4 objectAlbedo = UNITY_SAMPLE_TEX2D(_MainTex, mainUV) * _Color;
            ...
            UNITY_BRANCH if (_BaseTexturesTriplanar)
            {
                objectFrontUV = float2(IN.worldPos.x * -flipUV.z, IN.worldPos.y) * _MainTex_ST.xy + _MainTex_ST.zw;
                objectSideUV = float2(IN.worldPos.z * flipUV.x, IN.worldPos.y) * _MainTex_ST.xy + _MainTex_ST.zw;

                float4 objectAlbedoSide = UNITY_SAMPLE_TEX2D(_MainTex, objectSideUV) * _Color;
                float4 objectAlbedoFront = UNITY_SAMPLE_TEX2D(_MainTex, objectFrontUV) * _Color;
    ...
    if(_Terrain_Parallax)
    { 
        ParallaxUV(uvSplat, normalize(IN.tangentViewDir), origBlendMask, lod, tWeights.y);
    }
    #endif
    ...
    //====================================================================      	
                ...
                float tolWeightTop = saturate(tWeights.y + (1 - blendMask[0].r));
            #else
                float tolWeightTop = 1.0f;
            #endif

            SampleMask(mask, uvSplat, blendMask, tolWeightTop);
            SampleMaskTOL(mask_front, mask, uvSplat_front, tWeights.z);
            SampleMaskTOL(mask_side, mask, uvSplat_side, tWeights.x);
        #else
            SampleMask(mask, uvSplat, blendMask, tWeights.y);
            SampleMask(mask_front, uvSplat_front, blendMask, tWeights.z);
            SampleMask(mask_side, uvSplat_side, blendMask, tWeights.x);
        #endif 
        MaskWeight(mask, mask_front, mask_side, blendMask, tWeights, _HeightTransition);
    #else
        SampleMask(mask, uvSplat, blendMask, 1.0f);
            ...
            #ifdef _TERRAIN_TRIPLANAR_ONE 
                #ifndef TERRAIN_SPLAT_ADDPASS
                    tolWeightTop = saturate(tWeights.y + (1 - dBlendMask[0].r));
                #endif               
                float tolWeightFront = dBlendMask[0].r * tWeights.z;
                float tolWeightSide = dBlendMask[0].r * tWeights.x;

                SampleDistantMask(mask, dMask, uvStochR, uvStochG, uvScaled, dBlendMask, dMode, stochMask, tolWeightTop);
                SampleDistantMaskTOL(dMask_front, dMask, uvStochR_front, uvStochG_front, tolWeightFront, dMode, stochMask_front);
                SampleDistantMaskTOL(dMask_side, dMask, uvStochR_side, uvStochG_side, tolWeightSide, dMode, stochMask_side);
            #else
                SampleDistantMask(mask, dMask, uvStochR, uvStochG, uvScaled, dBlendMask, dMode, stochMask, tWeights.y);
                SampleDistantMask(mask_front, dMask_front, uvStochR_front, uvStochG_front, uvScaled_front, dBlendMask, dMode, stochMask_front, tWeights.z);
                SampleDistantMask(mask_side, dMask_side, uvStochR_side, uvStochG_side, uvScaled_side, dBlendMask, dMode, stochMask_side, tWeights.x);
            #endif
            MaskWeight(dMask, dMask_front, dMask_side, dBlendMask, tWeights, _Distance_HeightTransition);
        #else
            SampleDistantMask(mask, dMask, uvStochR, uvScaled, uvStochG, dBlendMask, dMode, stochMask, 1.0f);
        #endif
    #endif
    ...
    //-------------------- HEIGHTMAP OBJECT INTERSECTION ----------------
            ...
            UNITY_BRANCH if (_BaseTexturesTriplanar)
            {
                float4 objectMaskSide = UNITY_SAMPLE_TEX2D_SAMPLER(_MainMask, _MainTex, objectSideUV) * baseTriplanarWeights.x;
                float4 objectMaskFront = UNITY_SAMPLE_TEX2D_SAMPLER(_MainMask, _MainTex, objectFrontUV) * baseTriplanarWeights.z;
    ...
    //=======================================================================
        ...
        float3 puddleWeight = pow(abs(wNormal), 3.0f);
        ...
        horizontalWeight = smoothstep(0.9f, 1.0f, saturate(puddleWeight.y - ((puddleWeight.x + puddleWeight.z) * pow(2.0f, _PuddleHorizontalWeight * 10.0f))));
        ...
        { 
            ...
            {
                float raindropsDistance = smoothstep(_InTerra_GlobalRaindropsDistance.x, _InTerra_GlobalRaindropsDistance.y, distance(IN.worldPos, _WorldSpaceCameraPos));
    ...
    //=======================================================================
        ...
        #ifdef _TERRAIN_TRIPLANAR_ONE 
            #ifndef TERRAIN_SPLAT_ADDPASS
                tolWeightTop = saturate(tWeights.y + (1 - blendMask[0].r));
            #endif    
            SampleSplat(uvSplat, blendMask, tolWeightTop, mask, mixedDiffuse, mixedNormal, splat);
            SampleSplatTOL(frontDiffuse, frontNormal, uvSplat_front, blendMask, tWeights.z, mask);
            SampleSplatTOL(sideDiffuse, sideNormal, uvSplat_side, blendMask, tWeights.x, mask);
        #else
            SampleSplat(uvSplat, blendMask, tWeights.y, mask, mixedDiffuse, mixedNormal, splat);
            SampleSplat(uvSplat_front, blendMask, tWeights.z, mask, frontDiffuse, frontNormal, splat);
            SampleSplat(uvSplat_side, blendMask, tWeights.x, mask, sideDiffuse, sideNormal, splat);
        #endif       
    #else
        ...
        mixedNormal = TriplanarNormal(mixedNormal, worldTangent, worldBitangent, frontNormal, sideNormal, tWeights, flipUV);
    #endif
    ...
    //---------------------- DISTANCE BLENDING --------------------
        ...
        coverMask = 0;
            ...
            float3 dSideNormal;
                    ...
                    tolWeightTop = saturate(tWeights.y + (1 - dBlendMask[0].r));
                #endif
                SampleDistantSplat(uvStochR, uvStochG, uvScaled, dBlendMask, stochMask, tolWeightTop,
                                    dMask, dMode, distantDiffuse, distantNormal, coverMask);

                SampleDistantSplatTOL(dFrontDiffuse, dFontNormal, uvStochR_front, uvStochG_front,
                                        dBlendMask, stochMask_front, tWeights.z, dMask, dMode, coverMask);

                SampleDistantSplatTOL(dSideDiffuse, dSideNormal, uvStochR_side, uvStochG_side, 
                                    dBlendMask, stochMask_side, tWeights.x, dMask, dMode, coverMask) ;
            #else
                SampleDistantSplat(uvStochR, uvStochG, uvScaled, dBlendMask, stochMask, tWeights.y,
                                    dMask, dMode, distantDiffuse, distantNormal, coverMask);

                SampleDistantSplat(uvStochR_front, uvStochG_front, uvScaled_front, dBlendMask, stochMask_front,
                                    tWeights.z, dMask, dMode, dFrontDiffuse, dFontNormal, coverMask);

                SampleDistantSplat(uvStochR_side, uvStochG_side, uvScaled_side, dBlendMask, stochMask_side,
                                    tWeights.x, dMask, dMode, dSideDiffuse, dSideNormal, coverMask);
            ...
            distantNormal = TriplanarNormal(distantNormal, worldTangent, worldBitangent, dFontNormal, dSideNormal, tWeights, flipUV);
        #endif
    ...
    //===================================================================
        ...
        half3 tint = tex2D(_TerrainColorTintTexture, tintUV);
    #else
        half3 tint = UNITY_SAMPLE_TEX2D_SAMPLER(_TerrainColorTintTexture, _Splat0, tintUV);
        float tintDist = smoothstep(_TerrainColorTintDistance.x, _TerrainColorTintDistance.y, (distance(IN.worldPos, _WorldSpaceCameraPos)));
    ...
    { 
        ...
        { 
            ...
            trackDepth = UNITY_SAMPLE_TEX2D_SAMPLER(_InTerra_TrackTexture, _Splat0, trackUV);
            ...
            if (trackDepth.a > 0.0001f)
            {
                ...
                float4 heights[4];
                 heights[0] = UNITY_SAMPLE_TEX2D_SAMPLER(_InTerra_TrackTexture, _Splat0, trackUV + float2(0.0,  normalsOffset));
                 heights[1] = UNITY_SAMPLE_TEX2D_SAMPLER(_InTerra_TrackTexture, _Splat0, trackUV + float2( normalsOffset, 0.0));
                 heights[2] = UNITY_SAMPLE_TEX2D_SAMPLER(_InTerra_TrackTexture, _Splat0, trackUV + float2(-normalsOffset, 0.0));
                 heights[3] = UNITY_SAMPLE_TEX2D_SAMPLER(_InTerra_TrackTexture, _Splat0, trackUV + float2(0.0, -normalsOffset));
        ...
        /*#ifdef _PUDDLES
            ...
            {
                lightSlopeWeight = smoothstep(0.5f, 1.0f, saturate(puddleWeight.y - (puddleWeight.x + puddleWeight.z)));
                wettnessAroundPuddle = smoothstep(_InTerra_GlobalPuddles.y, _InTerra_GlobalPuddles.x, heightSum);
                float wetLightSlope = wettnessAroundPuddle * lightSlopeWeight;
                mixedDiffuse.a = lerp(mixedDiffuse.a, 0.65f, wetLightSlope);       
            }
        #endif*/
    ...
    //=======================================================================
        ...
        if (_Detail > 0.0f)
        {
            UNITY_BRANCH if (_BaseTexturesTriplanar)
            {
                detailUV = TRANSFORM_TEX(IN.worldPos.xz, _DetailAlbedoMap);
                detailFrontUV = float2(IN.worldPos.x * -flipUV.z, IN.worldPos.y) * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
                detailSideUV = float2(IN.worldPos.z * flipUV.x, IN.worldPos.y) * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
            }
            else
            {
                detailUV = TRANSFORM_TEX(IN.mainTC_tWeightY_hOffset.xy, _DetailAlbedoMap);
            }
            ...
            half3 detail = UNITY_SAMPLE_TEX2D_SAMPLER(_DetailAlbedoMap, _MainTex, detailUV);
            detailNormal = UnpackNormalWithScale(UNITY_SAMPLE_TEX2D_SAMPLER(_DetailNormalMap, _MainTex, detailUV), _DetailNormalMapScale);

            UNITY_BRANCH if (_BaseTexturesTriplanar)
            {
                float4 detailSide = UNITY_SAMPLE_TEX2D_SAMPLER(_DetailAlbedoMap, _MainTex, detailSideUV) * baseTriplanarWeights.x;
                float4 detailFront = UNITY_SAMPLE_TEX2D_SAMPLER(_DetailAlbedoMap, _MainTex, detailFrontUV) * baseTriplanarWeights.z;
                detail = detail * baseTriplanarWeights.y + detailSide + detailFront;

                float3 detailNormalSide = UnpackNormalWithScale(UNITY_SAMPLE_TEX2D_SAMPLER(_DetailNormalMap, _MainTex, detailSideUV), _DetailNormalMapScale);
                float3 detailNormalFront = UnpackNormalWithScale(UNITY_SAMPLE_TEX2D_SAMPLER(_DetailNormalMap, _MainTex, detailFrontUV), _DetailNormalMapScale);
            ...
            objectAlbedo.rgb = lerp(objectAlbedo.rgb, objectAlbedo.rgb * (detail * unity_ColorSpaceDouble.r), _DetailStrenght);
        }

        float3 objectNormal = UnpackNormalWithScale(UNITY_SAMPLE_TEX2D_SAMPLER(_BumpMap, _MainTex, mainUV), _BumpScale);

        UNITY_BRANCH if (_BaseTexturesTriplanar)
        {
            float3 objectNormalSide = UnpackNormalWithScale(UNITY_SAMPLE_TEX2D_SAMPLER(_BumpMap, _MainTex, objectSideUV), _BumpScale);
            float3 objectNormalFront = UnpackNormalWithScale(UNITY_SAMPLE_TEX2D_SAMPLER(_BumpMap, _MainTex, objectFrontUV), _BumpScale);
        ...
        UNITY_BRANCH if (_Detail > 0.0f)
        {
            objectNormal = (lerp(objectNormal, BlendNormals(objectNormal, detailNormal), _DetailStrenght));
        }

        UNITY_BRANCH if (!_GlobalWetnessDisabled)
        {
            objectNormal.xy = _InTerra_GlobalWetness > 0.4 ? objectNormal.xy * (1 - min(0.8f, (_InTerra_GlobalWetness * 2.25f - 1))) : objectNormal.xy;
        }
        ...
        mixedNormal = lerp(mixedNormal, objectNormal, heightIntersect.r);
            ...
            UNITY_BRANCH if (_EmissionEnabled > 0)
            {
                ...
                UNITY_BRANCH if (_BaseTexturesTriplanar)
                {
                    emissionUV = TRANSFORM_TEX(IN.worldPos.xz, _EmissionMap);                  
                }
                else
                {
                    emissionUV = TRANSFORM_TEX(IN.mainTC_tWeightY_hOffset.xy, _EmissionMap);
                }
                ...
                UNITY_BRANCH if (_BaseTexturesTriplanar)
                {
                    float2 emissionFrontUV = float2(IN.worldPos.x * -flipUV.z, IN.worldPos.y) * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
                    float2 emissionSideUV = float2(IN.worldPos.z * flipUV.x, IN.worldPos.y) * _EmissionMap_ST.xy + _EmissionMap_ST.zw;

                    float4 emissionSide = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, emissionSideUV) *  baseTriplanarWeights.x;
                    float4 emissionFront = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, emissionFrontUV) *  baseTriplanarWeights.z;
                    emissionTexture = emissionTexture * baseTriplanarWeights.y + emissionSide + emissionFront;
                }
    ...
    if (_InTerra_GlobalPuddles.y > 0.0f)
    {
        float lightSlopeWeight = smoothstep(0.0f, 1.0f, saturate(puddleWeight.y - ((puddleWeight.x + puddleWeight.z) * pow(2.0f, _PuddleWetnessHorizontalWeight * 10.0f))));
        float wettnessAroundPuddle = smoothstep(_InTerra_GlobalPuddles.y, _InTerra_GlobalPuddles.x, heightSum);

        float wetHorizontal = wettnessAroundPuddle * max(horizontalWeight, min(0.5, lightSlopeWeight));
