...
#endif
{
    ...
    blendMask[3] = float4(0, 0, 0, 0);
        ...
        #endif 
        #ifdef _LAYERS_SIXTEEN
            if (_NumLayersCount > 8) blendMask[2] = SAMPLE_TEXTURE2D(_Control2, sampler_Control, uvMainAndLM.xy);
            if (_NumLayersCount > 12) blendMask[3] = SAMPLE_TEXTURE2D(_Control3, sampler_Control, uvMainAndLM.xy);
        #endif
        #if defined(_LAYERS_EIGHT) || defined(_LAYERS_SIXTEEN)
    ...
    #if defined(_TERRAIN_BLEND_HEIGHT) && !defined(_TERRAIN_BASEMAP_GEN) && !defined(_LAYERS_ONE) && !(defined(_LAYERS_EIGHT) || defined(_LAYERS_SIXTEEN))
        if (_HeightmapBlending == 1)
        {
            //preventing the black area when more than one pass
            blendMask[0] = (blendMask[0].r + blendMask[0].g + blendMask[0].b + blendMask[0].a == 0.0f ? 1.000f : blendMask[0]); 
        }
    #endif

    #if !defined(_TERRAIN_BASEMAP_GEN)
        float3 flipUV = worldNormal.rgb < 0 ? -1 : 1;
        float3 tWeights = abs(worldNormal.rgb);
    ...
    //-------------------------  DISTANCE BLEND  ----------------------
    half4 origBlendMask[4] = blendMask;
    ...
    //-------------------------  PARALLAX  ----------------------
        ...
        #endif
            {
                lod =  smoothstep(_MipMapFade.x, _MipMapFade.y, distanceFromCamera);
            }
    #endif
    ...
    //================================================================================
            ...
            {
                objectFrontUV = float2(worldPos.x * -flipUV.z, worldPos.y) * _BaseMap_ST.xy + _BaseMap_ST.zw;
                objectSideUV = float2(worldPos.z * flipUV.x, worldPos.y) * _BaseMap_ST.xy + _BaseMap_ST.zw;

                float4 objectAlbedoSide = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, objectSideUV) * _BaseColor;
                float4 objectAlbedoFront = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, objectFrontUV) * _BaseColor;
                ...
                float4 objectMaskSide = SAMPLE_TEXTURE2D(_MainMask, sampler_BaseMap, objectSideUV) * baseTriplanarWeights.x;
                float4 objectMaskFront = SAMPLE_TEXTURE2D(_MainMask, sampler_BaseMap, objectFrontUV) * baseTriplanarWeights.z;
    ...
    //-------------------- PARALLAX OFFSET -------------------------                  
        ...
        UNITY_BRANCH if (_Terrain_Parallax == 1)
        {
            ...
            ParallaxUV(uvSplat, viewDirTerrain, origBlendMask, tWeights.y, lod);
        }       
    #endif
    ...
    //====================================================================================  
        ...
        #endif 
        MaskWeight(mask, mask_front, mask_side, blendMask, tWeights);
    #else
        SampleMask(mask, uvSplat, blendMask, 1.0f);
                ...
                SampleDistantMask(mask_front, dMask_front, uvStochR_front, uvStochG_front, uvScaled, dBlendMask, dMode, stochMask_front, tWeights.z);
            ...
            #endif
            MaskWeight(dMask, dMask_front, dMask_side, dBlendMask, tWeights);
        #else
    ...
    //=======================================================================
        ...
        { 
            if (puddlesHeight.x > 0.5f && _InTerra_GlobalRaindropsDistance.y > 0.0f)
            {
    ...
    //========================================================================================
        ...
        float3 sideNormal;
            ...
            #endif
            SampleSplat(uvSplat, blendMask, tolWeightTop, mask, mixedDiffuse, mixedNormal);
        ...
        #else
            SampleSplat(uvSplat, blendMask, tWeights.y, mask, mixedDiffuse, mixedNormal);
            SampleSplat(uvSplat_front, blendMask, tWeights.z, mask, frontDiffuse, frontNormal);
            SampleSplat(uvSplat_side, blendMask, tWeights.x, mask, sideDiffuse, sideNormal);
        #endif 
    #else
    ...
    if (_Tracks)
    {
        ...
        float2 trackUV = float2((worldPos.x - _InTerra_TrackPosition.x) + (halfTrackArea), 
                        -(worldPos.z - _InTerra_TrackPosition.z - (halfTrackArea))) * 1.0f / _InTerra_TrackArea;

        float trackDist = smoothstep(_InTerra_TrackArea - 1.0f, _InTerra_TrackArea - _InTerra_TrackFading, 
                        (distance(worldPos, float3(_WorldSpaceCameraPos.x, _WorldSpaceCameraPos.y, _WorldSpaceCameraPos.z))));
        ...
        float2 trackDetailUV = float2(tUV.x, -tUV.y) * _TrackDetailTexture_ST.xy + _TrackDetailTexture_ST.zw;
            ...
            float2 trackParallaxOffset = ParallaxOffset(_InTerra_TrackTexture, SamplerState_Linear_Repeat, _ParallaxTrackSteps,  -trackValues.y, trackUV, 
                                        float3( -viewDirTerrain.x, viewDirTerrain.y, -viewDirTerrain.z), _ParallaxTrackAffineSteps, _MipMapLevel 
                                        + (lod * (log2(max(_InTerra_TrackTexture_TexelSize.z, _InTerra_TrackTexture_TexelSize.w)) + 1)), 1 );
    ...
    mixedNormal.xy = _InTerra_GlobalWetness > 0.4 ? mixedNormal.xy * (1 - min(0.8f, (_InTerra_GlobalWetness * 2.25f - 1))) : mixedNormal.xy;
        ...
        if (_CheckHeight)
        {
            albedo *= float3(1.0f, 0.80f, 0.60f);
        }
    #endif
    ...
    //=======================================================================================
        ...
        UNITY_BRANCH if (_BaseTexturesTriplanar)
        {
            float3 objectNormalSide = UnpackNormals(SAMPLE_TEXTURE2D(_BumpMap, sampler_BaseMap, objectSideUV), _BumpScale);
            float3 objectNormalFront = UnpackNormals(SAMPLE_TEXTURE2D(_BumpMap, sampler_BaseMap, objectFrontUV), _BumpScale);
        ...
        UNITY_BRANCH if (_Detail > 0)
        {
            ...
            UNITY_BRANCH if (_BaseTexturesTriplanar)
            {
                detailFrontUV = float2(worldPos.x * -flipUV.z, worldPos.y) * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
                detailSideUV = float2(worldPos.z * flipUV.x, worldPos.y) * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
            }

            UNITY_BRANCH if (_HasDetailAlbedo > 0)
            {
                half3 detail = SAMPLE_TEXTURE2D(_DetailAlbedoMap, SamplerState_Linear_Repeat, detailUV).rgb;              

                UNITY_BRANCH if (_BaseTexturesTriplanar)
                {
                    float3 detailSide = SAMPLE_TEXTURE2D(_DetailAlbedoMap, SamplerState_Linear_Repeat, detailSideUV).rgb * baseTriplanarWeights.x;
                    float3 detailFront = SAMPLE_TEXTURE2D(_DetailAlbedoMap, SamplerState_Linear_Repeat, detailFrontUV).rgb * baseTriplanarWeights.z;
            ...
            half3 detailNormal = UnpackNormalScale(SAMPLE_TEXTURE2D(_DetailNormalMap, SamplerState_Linear_Repeat, detailUV), _DetailNormalMapScale);

            UNITY_BRANCH if (_BaseTexturesTriplanar)
            {
                float3 detailNormalSide = UnpackNormals(SAMPLE_TEXTURE2D(_DetailNormalMap, SamplerState_Linear_Repeat, detailSideUV), _DetailNormalMapScale);
                float3 detailNormalFront = UnpackNormals(SAMPLE_TEXTURE2D(_DetailNormalMap, SamplerState_Linear_Repeat, detailFrontUV), _DetailNormalMapScale);
        ...
        UNITY_BRANCH if (_EmissionEnabled > 0)
        {
            UNITY_BRANCH if (_BaseTexturesTriplanar)
            {
                float2 emissionFrontUV = float2(worldPos.x * -flipUV.z, worldPos.y) * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
                float2 emissionSideUV = float2(worldPos.z * flipUV.x, worldPos.y) * _EmissionMap_ST.xy + _EmissionMap_ST.zw;

                float3 emissionSide = SAMPLE_TEXTURE2D(_EmissionMap, SamplerState_Linear_Repeat, emissionSideUV).rgb * _EmissionColor.rgb * baseTriplanarWeights.x;
                float3 emissionFront = SAMPLE_TEXTURE2D(_EmissionMap, SamplerState_Linear_Repeat, emissionFrontUV).rgb * _EmissionColor.rgb * baseTriplanarWeights.z;
                objectEmission = objectEmission * baseTriplanarWeights.y + emissionSide + emissionFront;
            }
    ...
    //===============|   PUDDLES & RAINDROPS  FINAL MIX |======================
        ...
        {
            float lightSlopeWeight = smoothstep(0.0f, 1.0f, saturate(puddleWeight.y - ((puddleWeight.x + puddleWeight.z) * pow(2.0f, _PuddleWetnessHorizontalWeight * 10))));
            float wettnessAroundPuddle = smoothstep(_InTerra_GlobalPuddles.y, _InTerra_GlobalPuddles.x, heightSum);

            float wetHorizontal = wettnessAroundPuddle * max(horizontalWeight, min(0.65, lightSlopeWeight));
    ...
    mixedDiffuse = mixedDiffuse + SAMPLE_TEXTURE2D_LOD(_Splat0, sampler_Splat0, float2(0, 0), 10) * 1e-5f;
    mixedNormal.z = mixedNormal.z + SAMPLE_TEXTURE2D_LOD(_Splat0, sampler_Splat0, float2(0, 0), 10).x * 1e-5f;
    //==========================================================================
}
