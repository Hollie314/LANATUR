...
void NormalAndHeight_float(float4 uv, float3 positionWS, float3 wNormal, float3 wTangent, float3 wBTangent, float3 wDir, out float4 terrainNormals,  out float heightOffset, out float3 worldPosition, out float3 tangentViewDir, out float3 worldViewDir, out float3 worldNormal, out float3 worldTangent, out float3 worldBitangent, out float2 detailUV, out float2 mainUV, out float2 emissionUV)
{
    ...
    {
        mainUV = positionWS.xz * _BaseMap_ST.xy + _BaseMap_ST.zw;
        emissionUV = positionWS.xz * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
        detailUV = positionWS.xz * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
    }
    else
    {
        mainUV = uv.xy * _BaseMap_ST.xy + _BaseMap_ST.zw;
        emissionUV = uv.xy * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
        detailUV = uv.xy * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
    }

    #ifdef _OBJECT_PARALLAX
    UNITY_BRANCH if (_Object_Parallax == 1)
    { 
        ...
        float3 objectTangentViewDir = _BaseTexturesTriplanar ? tangentViewDir : mul(objectToTangent, worldViewDir);
        float2 objectParallaxOffset = ParallaxOffset(_MainMask, sampler_MainMask, _ParallaxSteps, _ParallaxHeight, mainUV, objectTangentViewDir, _ParallaxAffineSteps, _MipMapLevel + (lod * (log2(max(_MainMask_TexelSize.z, _MainMask_TexelSize.w)) + 1)), 0);

        mainUV += objectParallaxOffset;
        emissionUV += objectParallaxOffset * (_EmissionMap_ST.xy / _BaseMap_ST.xy);
        detailUV += objectParallaxOffset * (_DetailAlbedoMap_ST.xy / _BaseMap_ST.xy);
    }
    #endif
}