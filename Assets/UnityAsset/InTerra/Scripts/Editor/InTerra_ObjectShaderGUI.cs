...
namespace InTerra
{
	...
	{	
		...
		public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
		{
			...
			DictionaryMaterialMeshTerrain materialMeshTerrain = InTerra_Data.GetSceneData().MaterialMeshTerrain;
			Terrain[] terrains = Terrain.activeTerrains;

			if (terrains.Length == 0 && (materialMeshTerrain == null || materialMeshTerrain.Count == 0))
			{
				EditorGUILayout.HelpBox("There is no Terrain or Mesh Terrain in this Scene!", MessageType.Warning);
				correctTerrainPlacement = false;
			}
			else
			{
				correctTerrainPlacement = true;
				if (updateAtOpen)
				{
					if (!objectInfoInit)
					{
				...
				if(targetMat.GetFloat("_CustomTerrainSelection") == 0)
                {
					...
					{
						EditorGUILayout.HelpBox("The average position of the Objects using this Material isn't on Terrain with InTerra shader!", MessageType.Warning);
						correctTerrainPlacement = false;
					}
				}
			}
			...
			{ 
				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{
					...
					chRemapping = EditorGUILayout.Foldout(chRemapping, "Channels Remapping");
			...
			//=======================================================================
				...
				string alphaClipTip = "If enabled the transparent part of albedo texture will be cut off.";
				#if (USING_URP)
				if (targetMat.HasProperty("_AlphaClip"))
				{ 
					bool alphaClipURP = targetMat.GetFloat("_AlphaClip") == 1;
					using (new GUILayout.VerticalScope(EditorStyles.helpBox))
					{
						EditorGUI.BeginChangeCheck();
						alphaClipURP = EditorGUILayout.ToggleLeft(LabelAndTooltip("Alpha Clipping", alphaClipTip), alphaClipURP);
	
						if (EditorGUI.EndChangeCheck())
						{
							materialEditor.RegisterPropertyChangeUndo("Alpha Clipping");
							targetMat.SetFloat("_AlphaClip", alphaClipURP ? 1.0f : 0.0f);
							SetKeyword("_ALPHATEST_ON", alphaClipURP);
						}
					}
				}
				#endif
				#if (USING_HDRP)
				bool alphaClipHDRP = targetMat.GetFloat("_AlphaCutoffEnable") == 1;
				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{
					EditorGUI.BeginChangeCheck();
					alphaClipHDRP = EditorGUILayout.ToggleLeft(LabelAndTooltip("Alpha Clipping", alphaClipTip), alphaClipHDRP);
	
					if (EditorGUI.EndChangeCheck())
					{
						materialEditor.RegisterPropertyChangeUndo("Alpha Clipping");
						SetKeyword("_ALPHATEST_ON", alphaClipHDRP);
						targetMat.SetFloat("_AlphaCutoffEnable", alphaClipHDRP ? 1.0f : 0.0f);
														
						if (targetMat.GetFloat("_AlphaCutoffEnable") == 1)
						{
							targetMat.SetFloat("_ZTestGBuffer", 3);
						}
						else
						{
							targetMat.SetFloat("_ZTestGBuffer", 4);
						}
					}
				}
				#endif
			...
			//--------------------- OBJECT BASE TEXTURES TEXTURES TRIPLANAR -------------------------
			bool baseTexturesTriplanar = targetMat.GetFloat("_BaseTexturesTriplanar") == 1;
			bool triplanarFoldout = targetMat.GetFloat("_TriplanarFoldout") == 1;
			float baseTriplanarSharpness = targetMat.GetFloat("_BaseTexturesTriplanarSharpness");
			EditorGUI.BeginChangeCheck();
			using (new GUILayout.VerticalScope(EditorStyles.helpBox))
			{
				InTerra_GUI.FoldoutToggle("Triplanar Mapping (Object Textures)", "Applies triplanar mapping to Object textures.", ref baseTexturesTriplanar, ref triplanarFoldout);
				if (triplanarFoldout)
				{
					GUI.enabled = baseTexturesTriplanar;
					baseTriplanarSharpness = EditorGUILayout.Slider("Blending Sharpness", baseTriplanarSharpness, 2, 10);
					GUI.enabled = true;
				}
				if (EditorGUI.EndChangeCheck())
				{
					materialEditor.RegisterPropertyChangeUndo("InTerra Object Main Textures Triplanar");
					targetMat.SetFloat("_BaseTexturesTriplanar", baseTexturesTriplanar ? 1.0f : 0.0f);
					targetMat.SetFloat("_TriplanarFoldout", triplanarFoldout ? 1.0f : 0.0f);
					targetMat.SetFloat("_BaseTexturesTriplanarSharpness", baseTriplanarSharpness);
				}
			}
			...
			{
				bool parallax;
				bool parallaxFoldout;

				if (!globalData.disableObjectParallax)
				{
					parallax = targetMat.GetFloat("_Object_Parallax") == 1;
					parallaxFoldout = targetMat.GetFloat("_ParallaxFoldout") == 1;
				}
				else
				{
					parallax = false;
					parallaxFoldout = false;
					GUI.enabled = false;
				}
				...
				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{
					...
					InTerra_GUI.FoldoutToggle("Parallax Occlusion Mapping", "An illusion of 3D effect created by offsetting the texture depending on heightmap.", ref parallax, ref parallaxFoldout);

					if (EditorGUI.EndChangeCheck())
					{
						materialEditor.RegisterPropertyChangeUndo("InTerra Object Parallax");
						targetMat.SetFloat("_Object_Parallax", parallax ? 1.0f : 0.0f);
						targetMat.SetFloat("_ParallaxFoldout", parallaxFoldout ? 1.0f : 0.0f);
					}
			...
			//------------------------- DETAIL -------------------------
			bool detail = targetMat.GetFloat("_Detail") > 0 ? true : false;
			bool detailFoldout = targetMat.GetFloat("_DetailFoldout") > 0 ? true : false;

			using (new GUILayout.VerticalScope(EditorStyles.helpBox))
			{				
				EditorGUI.BeginChangeCheck();
				InTerra_GUI.FoldoutToggle("Detail Map", "Secondary textures.", ref detail, ref detailFoldout);

				if (EditorGUI.EndChangeCheck())
				{
					materialEditor.RegisterPropertyChangeUndo("InTerra Object DetailMap");
					targetMat.SetFloat("_Detail", detail ? 1.0f : 0.0f);
					targetMat.SetFloat("_DetailFoldout", detailFoldout ? 1.0f : 0.0f);
				}
			...
			if (detailFoldout) EditorGUILayout.Space();
			...
			if (targetMat.HasProperty("_EmissionEnabled"))
			{
				...
				bool emissionFoldout = targetMat.GetFloat("_EmissionFoldout") == 1;

				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{	
					EditorGUI.BeginChangeCheck();
					InTerra_GUI.FoldoutToggle("Emission", "Light Emission of Object.", ref emission, ref emissionFoldout);

					if (EditorGUI.EndChangeCheck())
					{
						materialEditor.RegisterPropertyChangeUndo("InTerra Emission");
						targetMat.SetFloat("_EmissionEnabled", emission ? 1.0f : 0.0f);
						targetMat.SetFloat("_EmissionFoldout", emissionFoldout ? 1.0f : 0.0f);
					}

					if (emissionFoldout)
					{
						GUI.enabled = emission;
			...
			//-------------------|   TERRAIN LAYERS    |----------------------
			//================================================================
			GUI.enabled = correctTerrainPlacement;

			using (new GUILayout.VerticalScope(EditorStyles.helpBox))
			{
				bool tlFoldout = targetMat.GetFloat("_TerrainLayersFoldout") == 1;
				EditorGUI.BeginChangeCheck();

				InTerra_GUI.CenteredFoldout("TERRAIN LAYERS", ref tlFoldout);

				if (EditorGUI.EndChangeCheck())
				{
					targetMat.SetFloat("_TerrainLayersFoldout", tlFoldout ? 1.0f : 0.0f);
				}
				...
				{ 
					...
					if (isOnTerrain && layers == NumberOfLayers.OnePass)
					{
						...
						#if (USING_URP || USING_HDRP)
							int layersInPass = targetMat.IsKeywordEnabled("_LAYERS_EIGHT") ? 8 : 4;
							#if USING_HDRP
								layersInPass = targetMat.IsKeywordEnabled("_LAYERS_SIXTEEN") ? 16 : layersInPass;
							#endif
						#else
							int layersInPass = globalData.eightLayersPass ? 8 : 4;
						#endif
						...
						if (terrain)
						{
							...
							if (!(targetMat.shader.name.Contains("InTerra/HDRP") || layersInPass == 8))
							{
								EditorGUI.BeginChangeCheck();
							...
							scrollLayers = EditorGUILayout.BeginScrollView(scrollLayers);
			...
			using (new GUILayout.VerticalScope(EditorStyles.helpBox))
			{
				bool intersectFoldout = targetMat.GetFloat("_TerrainIntersectionFoldout") == 1;
				EditorGUI.BeginChangeCheck();

				InTerra_GUI.CenteredFoldout("TERRAIN INTERSECTION", ref intersectFoldout);

				if (EditorGUI.EndChangeCheck())
				{
					targetMat.SetFloat("_TerrainIntersectionFoldout", intersectFoldout ? 1.0f : 0.0f);
				}
				...
				{ 
					...
					using (new GUILayout.VerticalScope(EditorStyles.helpBox))
					{
						...
						bool steepSlopesFoldout = targetMat.GetFloat("_SteepSlopesFoldout") == 1;
						EditorGUI.BeginChangeCheck();

						InTerra_GUI.CenteredFoldout("Steep slopes", ref steepSlopesFoldout);

						if (EditorGUI.EndChangeCheck())
						{
							targetMat.SetFloat("_SteepSlopesFoldout", steepSlopesFoldout ? 1.0f : 0.0f);
						}

						if(steepSlopesFoldout)
						{ 
							...
							if (EditorGUI.EndChangeCheck())
							{
								...
								targetMat.SetFloat("_SteepIntersection", steepIntersect ? 1.0f : 0.0f);
								targetMat.SetVector("_Intersection2", intersection2);
							}

							//------------------------- TERRAIN LAYERS TRIPLANAR --------------------------
							bool triplanar = targetMat.IsKeywordEnabled("_OBJECT_TRIPLANAR");
							bool disOffset = targetMat.GetFloat("_DisableOffsetY") == 1;

							EditorGUI.BeginChangeCheck();
							triplanar = EditorGUILayout.ToggleLeft(LabelAndTooltip("Triplanar Mapping (Terrain Layers)", "The Texture on steep slopes of Object will not be stretched."), triplanar);
							EditorGUI.indentLevel = 1;
							if (triplanar)
							{
					...
					if (terrainMaterial != null && terrainMaterial.IsKeywordEnabled("_TERRAIN_DISTANCEBLEND"))
					{
						using (new GUILayout.VerticalScope(EditorStyles.helpBox))
						{
							...
							EditorGUI.BeginChangeCheck();
							distanceBlend = EditorGUILayout.ToggleLeft(LabelAndTooltip("Disable Distance Blending (Hide Tiling)", "If \"Distance Blending\" in Terrain Material is set on, this option will disable it only for this Material to prevent additional samplings and calculations. This may cause some more or less visible seams in distance."), distanceBlend);

							if (EditorGUI.EndChangeCheck())
							{
								materialEditor.RegisterPropertyChangeUndo("InTerra Disable Distance Blending (Hide Tiling)");
			...
			if (targetMat.shader.name.Contains(InTerra_Data.HDRPObjectTessellationShaderName))
			{
				...
				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{
					bool tessellationFoldout = targetMat.GetFloat("_TessellationFoldout") == 1;
					EditorGUI.BeginChangeCheck();

					InTerra_GUI.CenteredFoldout("TESSELLATION", ref tessellationFoldout);

					if (EditorGUI.EndChangeCheck())
					{
						targetMat.SetFloat("_TessellationFoldout", tessellationFoldout ? 1.0f : 0.0f);
					}

					if (tessellationFoldout)
					{
						...
						using (new GUILayout.VerticalScope(EditorStyles.helpBox))
						{
							...
							if(terrain && terrain.materialTemplate.shader.name.Contains(InTerra_Data.TessellationShaderFolder))
							{
			...
			//------------------------------ DISABLE GLOBAL WETNESS -------------------------------
			if (targetMat.shader.name != InTerra_Data.DiffuseObjectShaderName)
			{
				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{
					bool distanceGS = targetMat.GetFloat("_GlobalWetnessDisabled") == 1;


					EditorGUI.BeginChangeCheck();
					distanceGS = EditorGUILayout.ToggleLeft(LabelAndTooltip("Don't apply Global Wetness", "If checked Global Wetness will not be applied on the Object."), distanceGS);

					if (EditorGUI.EndChangeCheck())
					{
						materialEditor.RegisterPropertyChangeUndo("InTerra Disable Global Wetness for Object");

						if (distanceGS)
						{
							targetMat.SetFloat("_GlobalWetnessDisabled", 1);
						}
						else
						{
							targetMat.SetFloat("_GlobalWetnessDisabled", 0);
						}
					}
				}
			}
					...
					using (new GUILayout.VerticalScope(EditorStyles.helpBox))
					{
						...
						if (matContainsInstance)
						{
							using (new GUILayout.HorizontalScope(EditorStyles.helpBox))
							{
								...
								EditorGUILayout.LabelField("There are some Instanced Materials with InTerra shader, if you want to use <b>Multiple Terrains Material</b> options, no Instanced Materials with  <b><i>Object Into Terrain Integration</i></b> shader are allowed.");
							}
							GUI.enabled = false;
						}
						...
						{
							EditorGUILayout.HelpBox("There are some Terrains with identical names. Unique name for each Terrain is required.", MessageType.Warning);
							GUI.enabled = false;
						}

						//-----------------------|  INVALID CHARACTERS IN TERRAINS NAMES  |-------------------
						if (terrainsInvalidChars)
						{
							EditorGUILayout.HelpBox("There are invalid characters in some of the Terrains names. Only characters that are allowed for file names are allowed for Terrains names.", MessageType.Warning);
							GUI.enabled = false;
						}
						...
						if (targetMat.GetTag("BaseName", false).Length > 0 )
						{
							...
							{
								InTerra_MaterialManager.MaterialTagWarnings(terrain, targetMat, rendTerrain, relatedMaterials);
								EditorGUILayout.Space();
							}
			...
			using (new GUILayout.VerticalScope(EditorStyles.helpBox))
			{
				if (GUILayout.Button("Update Terrain Data", styleButtonBold))
				{
					...
					if (terrains.Length > 0 || InTerra_Data.GetSceneData().MeshTerrainsList.Count > 0 && materialTerrain.ContainsKey(targetMat))
					{
						GetTerrain();
						CreateObjectsLists(targetMat, terrain);
						InTerra_MaterialManager.CreateRelatedMaterialsList(baseName, relatedMaterials);
				...
				if (rend != null && rend.transform.position != null)
				{
					foreach (Material mat in rend.sharedMaterials)
					{
						if (mat != null && mat.shader != null && mat.shader.name != null && InTerra_Data.CheckObjectShader(mat))
						{
							...
							foreach (var meshTerrain in InTerra_Data.GetSceneData().MeshTerrainsList)
							{
