...
void NormalAndHeight_float(float4 uv, float3 worldPos, float3 wNormal, float3 wTangent, float3 wBitangent, float3 wDir, out float4 terrainNormals, out float heightOffset, out float3 tangentViewDir, out float3 worldViewDir, out float3 worldNormal, out float3 worldTangent, out float3 worldBitangent, out float2 detailUV, out float2 mainUV, out float2 emissionUV)
{
    ...
    {
        mainUV = worldPos.xz * _BaseColorMap_ST.xy + _BaseColorMap_ST.zw;
        emissionUV = worldPos.xz * _EmissiveColorMap_ST.xy + _EmissiveColorMap_ST.zw;
        detailUV = worldPos.xz * _DetailMap_ST.xy + _DetailMap_ST.zw;
    }
    else
    {
    ...
    UNITY_BRANCH if(_Object_Parallax)
    { 
        ...
        float2 objectParallaxOffset = ParallaxOffset(_MaskMap, sampler_MaskMap, _ParallaxSteps, _ParallaxHeight, mainUV, objectTangentViewDir, _ParallaxAffineSteps, _MipMapLevel + (lod * (log2(max(_MaskMap_TexelSize.z, _MaskMap_TexelSize.w)) + 1)), 0);
