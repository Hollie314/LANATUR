...
#define TessellationHeight(i) (mask[i] * DiffuseRemap(i).w * 0.01) + ((_DiffuseRemapOffset##i.z * 100) - ((DiffuseRemap(i).w * 0.01)/2));

void SampleHeights(out float4 mask[_LAYER_COUNT], float2 uv[_LAYER_COUNT], float4 blendMask[4], float lod)
{
    ...
    SampleHeight(0, blendMask[0].r);
        ...
        SampleHeight(1, blendMask[0].g);
            ...
            SampleHeight(3, blendMask[0].a);
                ...
                SampleHeight(8, blendMask[2].r);
                SampleHeight(9, blendMask[2].g);
                SampleHeight(10, blendMask[2].b);
                SampleHeight(11, blendMask[2].a);

                SampleHeight(12, blendMask[3].r);
                SampleHeight(13, blendMask[3].g);
                SampleHeight(14, blendMask[3].b);
                SampleHeight(15, blendMask[3].a);
            #endif
        #endif
    #endif
#undef SampleHeight
...
void SampleHeightTOL(out float4 mask[_LAYER_COUNT], float4 noTriplanarMask[_LAYER_COUNT], float2 uv[_LAYER_COUNT], float lod)
{
    ...
    #else
        void Tessellation_float(float4 worldNormal, float3 worldPos, out float3 displacement, out float tessFactor)
        {
    #endif   
#endif
    ...
    heightSum = HeightSum(mask, blendMask);
    ...
    #define TessllationHeight(i, blendMask)                             \
        UNITY_BRANCH if (blendMask > 0 && _LayerHasMask##i > 0 )        \
        {                                                               \
            mask[i] = (_InTerra_GlobalPuddles.x * horizontalWeight > mask[i]) ? \
                      ( lerp(mask[i] , _InTerra_GlobalPuddles.x, clamp(puddlesHeight.r, 0, 1) ) ) : mask[i]; \
        ...
        TessllationHeight(0, blendMask[0].r);
            ...
            TessllationHeight(1, blendMask[0].g);
                ...
                #endif
                #ifdef _LAYERS_SIXTEEN
                    TessllationHeight(8, blendMask[2].r);
                    TessllationHeight(9, blendMask[2].g);
                    TessllationHeight(10, blendMask[2].b);
                    TessllationHeight(11, blendMask[2].a);

                    TessllationHeight(12, blendMask[3].r);
                    TessllationHeight(13, blendMask[3].g);
                    TessllationHeight(14, blendMask[3].b);
                    TessllationHeight(15, blendMask[3].a);
                #endif
            #endif
        #endif
    ...
    if (_Tracks == 1)
    {
        ...
        trackDepth = SAMPLE_TEXTURE2D_LOD(_InTerra_TrackTexture, SamplerState_Linear_Repeat, trackUV, 0).r;
    ...
    #ifdef INTERRA_OBJECT
        float objectHeightMap = SAMPLE_TEXTURE2D_LOD(_MaskMap, sampler_MaskMap, mUV.xy * _BaseColorMap_ST.xy + _BaseColorMap_ST.zw, _MipMapLevel + (lod * _MipMapCount)).b * _MaskMapRemapScale.b + _MaskMapRemapOffset.b;
        ...
        displacement = terrainHeightSum.xxx;
            ...
            displacement *= float3(worldNormal.x, worldNormal.y * triplanarWeights.y, worldNormal.z);
        #else
