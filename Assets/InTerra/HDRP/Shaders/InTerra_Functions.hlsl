...
//------------------------------
    ...
    float3 TriplanarNormal(float3 normal, float3 tangent, float3 bitangent, float3 normal_front, float3 normal_side, float3 triplanarWeights, half3 flipUV)
    {
            ...
            normal_side.xy = mul(float2x2(tangent.yz, bitangent.yz), normal_side.xy);
        #else
    ...
    float2 random2(float2 st) {
        ...
        #define DiffuseRemap(i) float4(_DiffuseRemapScale##i.xyzw + _DiffuseRemapOffset##i.xyzw)
            ...
            void ParallaxUV(inout float2 uv[_LAYER_COUNT], float3 tangentViewDir, float4 blendMask[4], float weight, float lod)
            {

                #define uvParallax(i, blendMask)                                \
                UNITY_BRANCH if (blendMask * weight  > 0.01f)                   \
                {                                                               \
                    uv[i] += ParallaxOffset(_Mask##i, sampler_Splat0, _DiffuseRemapOffset##i.w, DiffuseRemap(i).w, uv[i], tangentViewDir,       \
                    _ParallaxAffineStepsTerrain, _MipMapLevel +  (lod * (log2(max(_Mask##i##_TexelSize.z, _Mask##i##_TexelSize.w)) + 1)), 0);   \
                }                                                   
    ...
    float3 BlendNormals(float3 n1, float3 n2)
    {
            ...
            #define SampleNormals(i, uv, ddx, ddy) (UnpackNormals(SAMPLE_TEXTURE2D_ARRAY_GRAD(_NormalArray16, sampler_Splat0, uv, i, ddx, ddy), _NormalScale##i).xyz)
        #else           
            #define SampleNormals(i, uv, ddx, ddy) (UnpackNormals(SAMPLE_TEXTURE2D_GRAD(_Normal##i, sampler_Splat0, uv, ddx, ddy), _NormalScale##i).xyz)
        #endif
    #elif defined(_TERRAIN_NORMAL_IN_MASK) 
        #define SampleNormals(i, uv, ddx, ddy) float3(UnpackNormalGAWithScale(mask[i], _NormalScale##i, _LayerHasMask##i).xyz)
    #else
        #define SampleNormals(i, uv, ddx, ddy) float3(0, 0, 1)
    #endif
    ...
    float3 SmoothMaskOrAlbedo(half mask, half albedo, float hasMask, float smoothness)
    {
        ...
        #define Smoothness(i, albedo) albedo.a *= _Smoothness##i
            ...
            #define sUV(i) ObjectSideUV(posOffset.z, _SplatUV##i, offsetX + (_DiffuseRemapScale##i.w * 0.004 * _SplatUV##i.y) * -flip.x);
                ...
                #define fUV(i) ObjectFrontUV(posOffset.x, _SplatUV##i, offsetZ + ( _TerrainTessOffset) * flip.z);
                #define sUV(i) ObjectSideUV(posOffset.z, _SplatUV##i, offsetX + (_TerrainTessOffset) * flip.x);
    ...
    void CoverMaskAndMode(float4 blendMask[4], out float coverMask, out int mode[_LAYER_COUNT])
    {
        coverMask = 0;
    ...
    float4 RemapMasks(float4 mask, float4 remapScale, float4 remapOffset)
    {
    ...
    #ifdef TERRAIN_MASK
        #define Mask(i, uv, ddx, ddy) SAMPLE_TEXTURE2D_GRAD(_Mask##i, sampler_Splat0, uv, ddx, ddy)
        ...
        #endif
    #else
        #define Mask(i, uv, ddx, ddy) float4(_Metallic##i, 1, 0.5, 0);
    ...
    #define SampleMasks(i, blendMask)                                   \
        ...
        {                                                               \
            dx = ddx(uv[i]);                                            \
            dy = ddy(uv[i]);                                            \
            mask[i] = RemapMask(i, Mask(i, uv[i], dx, dy));             \
        }                                                               \
        else                                                            \
        {                                                               \
        ...
        #undef SampleMasks
         ...
         #define SampleSplats(i, uv, ddx, ddy) SAMPLE_TEXTURE2D_ARRAY_GRAD(_SplatArray16, sampler_Splat0, uv, i, ddx, ddy)
    #else
        #define SampleSplats(i, uv, ddx, ddy) SAMPLE_TEXTURE2D_GRAD(_Splat##i, sampler_Splat0, uv, ddx, ddy)
    #endif
    ...
    void SampleSplat(float2 uv[_LAYER_COUNT], float4 blendMask[4], float weight, inout float4 mask[_LAYER_COUNT], out float4 mixAlbedo, out float3 mixNormal)
    {
        ...
        {                                                               \
            dx = ddx(uv[i]);                                            \
            dy = ddy(uv[i]);                                            \
            albedo[i] = SampleSplats(i, uv[i], dx, dy);                 \
            albedo[i].rgb *= DiffuseRemap(i).xyz;                       \
            albedo[i].a = Smoothness(i, albedo[i]).x;                   \
            normal[i] = SampleNormals(i, uv[i], dx, dy).xyz;            \
    ...
    void SampleDistantMask(float4 mask[_LAYER_COUNT], out float4 distMask[_LAYER_COUNT], float2 uvR[_LAYER_COUNT], float2 uvG[_LAYER_COUNT], float2 uv[_LAYER_COUNT], float4 blendMask[4], int mode[_LAYER_COUNT], inout float2 stochasticMask[_LAYER_COUNT], float weight)
    {          
        float2 sMask;
        float2 dx, dy;
        ...
        {                                                                                   \
            dx = ddx(uv[i]);                                                                \
            dy = ddy(uv[i]);                                                                \
            distMask[i] = sMask.r * RemapMask(i, Mask(i, uvR[i], dx, dy));                  \
            UNITY_BRANCH if (mode[i] > 0)                                                   \
            {                                                                               \
                distMask[i] +=  sMask.g * RemapMask(i, Mask(i, uvG[i], dx, dy));            \
            }                                                                               \
        }                                                           \
        else                                                        \
        {                                                           \
    ...
    void SampleDistantSplat(float2 uvR[_LAYER_COUNT], float2 uvG[_LAYER_COUNT], float2 uv[_LAYER_COUNT], float4 blendMask[4], float2 stochasticMask[_LAYER_COUNT], float weight, inout float4 mask[_LAYER_COUNT], int mode[_LAYER_COUNT], out float4 mixAlbedo, out float3 mixNormal, in out half coverMask)
    { 
        float3 normal[_LAYER_COUNT]; 
        ...
        {                                                               \
            dx = ddx(uv[i]);                                            \
            dy = ddy(uv[i]);                                            \
            albedoR = SampleSplats(i, uvR[i], dx, dy);                  \
            albedoR.rgb *=  DiffuseRemap(i).xyz;                        \
            albedoR.a = Smoothness(i, albedoR).x;                       \
            albedoR *= sMask.r;                                         \
            albedo[i] = albedoR;                                        \
            normal[i] = SampleNormals(i, uvR[i], dx, dy).xyz * sMask.r; \
                                                                        \
            UNITY_BRANCH if (mode[i] > 0)                               \
            {                                                           \
                albedoG = SampleSplats(i, uvG[i], dx, dy);              \
                albedoG.rgb *=  DiffuseRemap(i).xyz;                    \
                albedoG.a = Smoothness(i, albedoG).x;                   \
                albedoG *= sMask.g;                                     \
                albedo[i] = albedoR + albedoG;                          \
                normal[i] += SampleNormals(i, uvG[i], dx, dy) * sMask.g;\
            }                                                           \
            coverMask += blendMask;                                     \
    ...
    void MaskWeight(inout float4 mask[_LAYER_COUNT], float4 mask_front[_LAYER_COUNT], float4 mask_side[_LAYER_COUNT], float4 blendMask[4], inout float3 triplanarWeights, float heightBlendingSharpness)
    {
        ...
        for (int i = 0; i < _LAYER_COUNT; ++i)
        {
            ...
            if (_HeightmapBlending == 1)
            {
                triplanarWeights.rgb *= (1 / (pow(2, (heights + triplanarWeights) * (-min(heightBlendingSharpness, 45)))) + 1) * 0.5;
    ...
    void UnpackTrackSplatColor(out float4 trackSplatsColor[_LAYER_COUNT])
    {
        ...
        void SampleSplatTOL(in out float4 mixedAlbedo, in out float3 mixedNormal, float2 uv[_LAYER_COUNT], float4 blendMask[4], float weight, float4 mask[_LAYER_COUNT])
        {
            ...
            {          
                albedo[0] = SAMPLE_TEXTURE2D_GRAD(_Splat0, SamplerState_Linear_Repeat, uv[0], dx, dy);
                albedo[0].rgb *= DiffuseRemap(0).rgb;
                albedo[0].a = Smoothness(0, albedo[0]).x;
                normal[0] = SampleNormals(0, uv[0], dx, dy);
            }
        ...
        void SampleMaskTOL(out float4 mask[_LAYER_COUNT], float4 noTriplanarMask[_LAYER_COUNT], float2 uv[_LAYER_COUNT], float weight)
        {
            UNITY_BRANCH if (weight > 1e-5f)
            {
                float2 dx = ddx(uv[0]);
                float2 dy = ddy(uv[0]);
                mask[0] = RemapMask(0, Mask(0, uv[0], dx, dy));
            }
            else
            {
        ...
        void SampleDistantSplatTOL(in out float4 mixedAlbedo, in out float3 mixedNormal, float2 uvR[_LAYER_COUNT], float2 uvG[_LAYER_COUNT], float2 uv[_LAYER_COUNT], float4 blendMask[4], float2 stochasticMask[_LAYER_COUNT], float weight, float4 mask[_LAYER_COUNT], int mode[_LAYER_COUNT], in out half coverMask)
        {
            float4 albedo[1];
            float3 normal[1];
             ...
             blendMask[0].r *= weight;
                ...
                UNITY_BRANCH if (blendMask[0].r > 1e-5f && mode[0] != 3)
                {
                    ...
                    albedoR = SAMPLE_TEXTURE2D_GRAD(_Splat0, sampler_Splat0, uvR[0], dx, dy);
                    albedoR.rgb *= DiffuseRemap(0).xyz;
                    albedoR.a = Smoothness(0, albedoR).r;
                    albedoR *= sMask.r;
                    albedo[0] = albedoR;
                    normal[0] = sMask.r * SampleNormals(0, uvR[0], dx, dy);

                    if (mode[0] > 0)
                    {
                        albedoG = SAMPLE_TEXTURE2D_GRAD(_Splat0, sampler_Splat0, uvG[0], dx, dy);
                        albedoG.rgb *= DiffuseRemap(0).xyz;
                        albedoG.a = Smoothness(0, albedoG).r;
                        albedoG *= sMask.g;
                        albedo[0] = albedoR + albedoG;
                        normal[0] += sMask.g * SampleNormals(0, uvG[0], dx, dy);
                    }
        ...
        void SampleDistantMaskTOL(out float4 mask[_LAYER_COUNT], float4 noTriplanarMask[_LAYER_COUNT], float2 uvR[_LAYER_COUNT], float2 uvG[_LAYER_COUNT], float2 uv[_LAYER_COUNT], float weight, int mode[_LAYER_COUNT], float2 stochasticMask[_LAYER_COUNT])
        {
            ...
            UNITY_BRANCH if (weight > 1e-5f && mode[0] != 3 && _LayerHasMask0 > 0)
            {
                ...
                mask[0] = stochasticMask[0].r * RemapMask(0, Mask(0, uvR[0], dx, dy));
                if (mode[0] > 0)
                {
                    mask[0] += stochasticMask[0].g * RemapMask(0, Mask(0, uvG[0], dx, dy));
                }
            }
